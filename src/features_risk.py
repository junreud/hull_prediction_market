...
logger = get_logger(log_file="logs/features_lisk.log", level="INFO")


class FeatureRiskEngineering:
    """
    ...
    def create_time_period_features(
        ...
        logger.info(f"Created time period features: {df.columns}")
    ...
    def select_features_weighted_ensemble(
        ...
        for method, weight in weights.items():
            ...
            if method == 'correlation':
                # spearman : 순위 기반 상관(rank correlation) -> 이상치 영향이 적음.
                scores = df[feature_cols + [target_col]].corr(method="spearman")[target_col].abs()
...
def create_feature_engineering(
    ...
    FeatureRiskEngineering
        Configured instance
    """
    return FeatureRiskEngineering(config_path)
